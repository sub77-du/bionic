...
static int do_nftw(const char *path,
  ...
  // Translate to fts_open flags.
  int fts_flags = FTS_LOGICAL | FTS_COMFOLLOW | FTS_NOCHDIR;
  if (nftw_fn) {
    fts_flags = FTS_COMFOLLOW | ((nftw_flags & FTW_PHYS) ? FTS_PHYSICAL : FTS_LOGICAL);
    if (!(nftw_flags & FTW_CHDIR)) fts_flags |= FTS_NOCHDIR;
    if (nftw_flags & FTW_MOUNT) fts_flags |= FTS_XDEV;
  }
  bool postorder = (nftw_flags & FTW_DEPTH) != 0;

  // Call fts_open.
  char* const paths[2] = { const_cast<char*>(path), nullptr };
  FTS* fts = fts_open(paths, fts_flags, nullptr);
  if (fts == nullptr) {
    return -1;
  }
...
int nftw(const char* path, int (*nftw_fn)(const char*, const struct stat*, int, struct FTW*),
